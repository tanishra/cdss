...
class PDFService:
    ...
    def email_diagnosis_report(
        ...
        try:
            ...
            if not sender_email or not sender_password:
                logger.error("email_config_missing", error="SENDER_EMAIL or SENDER_PASSWORD not set")
                return False
    ...
    Clinical Team"""
            ...
            pdf_attachment.add_header('Content-Disposition', 'attachment', 
                                    filename=f'diagnosis_report_{patient_name.replace(" ", "_")}.pdf')
            msg.attach(pdf_attachment)
            
            # Send email
            server = smtplib.SMTP(smtp_server, smtp_port)
            server.starttls()
            server.login(sender_email, sender_password)  # Use direct login
